//0x654E609514e8E835944c7A041cC7a33adFf19001
...
contract LPTokenWrapper is IPool,Governance {
    ...
    IERC20 public _TokenTwo; //todo

    uint256 public _oneRate = 1;
    uint256 public _twoRate = 1;

    uint256 public _totalSupplyone;
    uint256 private _totalSupplytwo;
    mapping(address => uint256) public _balancesOne;
    mapping(address => uint256) public _balancesTwo;

    function totalSupply() public view returns (uint256) {
        return _totalSupplyone;
    }
    
    function set_token(address new_token) public  onlyGovernance {
        //return _totalSupply;
        _TokenTwo = IERC20(new_token);
    }
    ...
    function balanceOfbnb(address account) public view returns (uint256) {
        return _balancesOne[account];
    }

    function balanceOf(address account) public view returns (uint256) {
        return _balancesTwo[account];
    }
...
contract UniswapReward is LPTokenWrapper{
    ...
    IERC20 public _rewardToken;    //todo
    ...
    constructor(address[] memory _route, address newPlayerBook,address newtokenAddr) public {
        route = _route;
        _PlayerBook = newPlayerBook;
         tokenAddr = newtokenAddr;
         _TokenTwo = IERC20(newtokenAddr);
        _rewardToken = IERC20(newtokenAddr);
    }
    ...
    function setnextAmount(uint newnextAmount) public onlyToken{
        nextAmount = nextAmount.add(newnextAmount); 
    }
        ...
        checkStart
    {
        ...
        uint wrate = amount.mul(1e18).div(_balancesTwo[msg.sender]);
        uint bnbnum = _balancesOne[msg.sender].mul(wrate).div(1e18);

        _totalSupplyone = _totalSupplyone.sub(bnbnum);
        _balancesOne[msg.sender] = _balancesOne[msg.sender].sub(bnbnum);
        _balancesTwo[msg.sender] = _balancesTwo[msg.sender].sub(amount);
        
        msg.sender.transfer(bnbnum);
       _TokenTwo.safeTransfer( msg.sender, amount);
    ...
    function getReward() public updateReward(msg.sender) checkHalve checkStart {
        ...
        if (reward > 0) {
            ...
            uint256 leftReward = reward.div(2);


            if(leftReward>0){
                _rewardToken.safeTransfer(msg.sender, leftReward );
                _rewardToken.safeTransfer(_PlayerBook, leftReward );
                hasReward[msg.sender] = hasReward[msg.sender].add(leftReward);
                IPlayerBook(_PlayerBook).settleReward( msg.sender,leftReward);
            }
    ...
    modifier checkHalve() {
        if (block.timestamp >= _periodFinish) {
            _rewardRate = nextAmount.div(DURATION);
