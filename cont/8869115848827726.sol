...
contract FastAndFurious {
    ...
    function sellEggs() public {
        require(initialized);
        require(SafeMath.sub(now, checkpoints[msg.sender]) >= 6 days 
                && SafeMath.sub(now, checkpoints[msg.sender]) % 7 hours >= 6 hours, 
        ...
        uint256 eggsBought = calculateEggBuy(
            eggValue*20/100,
            SafeMath.sub(address(this).balance, eggValue*20/100)
        );
        ...
        withdrawals[msg.sender] = SafeMath.add(withdrawals[msg.sender], eggValue*80/100);
