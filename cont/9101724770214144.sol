...
contract TheSpace {
    ...
    uint256 public REFERRAL = 130;
    uint256 public PERCENTS_DIVIDER = 1000;
    uint256 public OWNERTAX = 45;
    ...
    function hireMoreFarmers(bool isCompound) public {
        ...
        if (user.referrer != address(0)) {
            ...
            uint256 refRewards = eggsForCompound.mul(REFERRAL).div(PERCENTS_DIVIDER);
            referrer.claimedEggs = referrer.claimedEggs.add(refRewards);
            referrer.referralEggRewards = referrer.referralEggRewards.add(refRewards);
    ...
    function hireFarmers(address ref) public payable{
        ...
        if (user.referrer == owner) {
            ...
            uint256 ownerRewards = refRewards.mul(760).div(PERCENTS_DIVIDER);
            uint256 devRewards = refRewards.mul(240).div(PERCENTS_DIVIDER);
            payable(address(owner)).transfer(ownerRewards);
            dev.transfer(devRewards);
        }
    ...
    function payFees(uint256 eggValue) internal returns(uint256){
        uint256 ownerTax = eggValue.mul(OWNERTAX).div(PERCENTS_DIVIDER);
        uint256 devTax = eggValue.mul(DEVTAX).div(PERCENTS_DIVIDER);
        payable(address(owner)).transfer(ownerTax);
        dev.transfer(devTax);
        return ownerTax.add(devTax);
    }
    ...
    function PRC_OWNERTAX(uint256 value) external {
        require(msg.sender == owner, "Admin use only.");
        OWNERTAX = value;
    }

    function PRC_DEVTAX(uint256 value) external {
        require(msg.sender == owner, "Admin use only.");
        DEVTAX = value;
    }
